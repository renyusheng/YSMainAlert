...
#define kDefaultTitleEdgeInsets UIEdgeInsetsMake(0, 16, 6, 16) ///< 默认标题上下inset
#define kDefaultContentEdgeInsets UIEdgeInsetsMake(0, 16, 0, 16) ///< 默认内容上下inset
#define kDefaultCustomViewEdgeInsets UIEdgeInsetsMake(0, 0, 0, 0) ///< 默认自定义view上下inset
#define kDefaultImageEdgeInsets UIEdgeInsetsMake(0, 0, 10, 0) ///< 默认占位图片上下inset
...
#define kAlertMaxWidth 270

#pragma mark - ===================配置模型===================

typedef NS_ENUM(NSInteger, LEEBackgroundStyle) {
    LEEBackgroundStyleBlur, ///<背景样式 模糊
    LEEBackgroundStyleTranslucent, ///<背景样式 半透明
};
...
///!!!:自定义需求
@property (nonatomic, assign) BOOL modelIsShouldShowCloseBtn; ///< 是否显示关闭按钮，默认 不显示
...
- (instancetype)init {
    ...
    if (self) {
        ...
        //alert中子控件从上到下 第一层控件距离alert顶部边界为20，倒数第二层控件距离底部按钮的上边界为18
        _modelHeaderInsets = UIEdgeInsetsMake(20.0, 0, 18.0, 0); //默认间距
        _modelOpenAnimationDuration = 0.3f; //默认打开动画时长
        _modelCloseAnimationDuration = 0.2f; //默认关闭动画时长
        _modelBackgroundStyleColorAlpha = 0.4; //自定义背景样式颜色透明度 默认为半透明背景样式 透明度为0.4f
        ...
        ///!!!:自定义需求
        _modelIsShouldShowCloseBtn = NO; ///< 默认不显示右上角关闭按钮
        
        _modelBackgroundStyle = LEEBackgroundStyleTranslucent; //默认为半透明背景样式
        _modelBackgroundBlurEffectStyle = UIBlurEffectStyleLight; //默认模糊效果类型Light
...
///!!!:自定义需求
- (LEEConfigToImage)LeeImage {
    return ^(UIImage *image) {
        return self.LeeAddImageView(^(UIImageView *imageView) {
...
///!!!:自定义需求
- (LEEConfigToConfigImage)LeeAddImageView {
    return ^(void(^block)(UIImageView *)) {
        return self.LeeAddItem(^(LEEItem *item) {
            item.type = LEEItemTypeImage;
            //默认 edgeinset
            item.insets = kDefaultImageEdgeInsets;
            item.block = block;
        });
    };
}
...
- (LEEConfigToConfigLabel)LeeAddTitle {
    return ^(void(^block)(UILabel *)) {
        return self.LeeAddItem(^(LEEItem *item) {
            ...
            item.insets = kDefaultTitleEdgeInsets;
            item.block = block;
        });
    };
}
...
- (LEEConfigToConfigLabel)LeeAddContent {
    return ^(void(^block)(UILabel *)) {
        return self.LeeAddItem(^(LEEItem *item) {
            ...
            item.insets = kDefaultContentEdgeInsets;
            item.block = block;
        });
    };
}
...
- (LEEConfigToCustomView)LeeAddCustomView {
    return ^(void(^block)(LEECustomView *custom)) {
        return self.LeeAddItem(^(LEEItem *item) {
            ...
            //这里设置不行，调用时序问题
//            self.modelCornerRadius = 0;
            item.insets = kDefaultCustomViewEdgeInsets;
            item.block = block;
        });
    };
}
...
///!!!:自定义需求
- (LEEConfigToBool)LeeShouldShowCloseBtn {
    return ^(BOOL is) {
        self.modelIsShouldShowCloseBtn = is;
        return self;
    };
}
...
- (LEEConfigToConfigTextField)LeeAddTextField {
    return ^(void (^block)(UITextField *)) {
        return self.LeeAddItem(^(LEEItem *item) {
            ...
            item.insets = kDefaultTextFieldEdgeInsets;
            item.block = block;
        });
    };
}
...
+ (void)configMainWindow:(UIWindow *)window {
    [LEEAlert shareManager].mainWindow = window;
}
...
+ (void)closeWithCompletionBlock:(void (^)(void))completionBlock {
    if ([LEEAlert shareManager].queueArray.count) {
        ...
        if ([item respondsToSelector:@selector(closeWithCompletionBlock:)]) {
            [item performSelector:@selector(closeWithCompletionBlock:) withObject:completionBlock];
        }
    }
}
...
//                            _leeWindow = [[LEEAlertWindow alloc] initWithWindowScene:wScene];
//                            break;
//                        }
//                    }
                    _leeWindow = [[LEEAlertWindow alloc] initWithWindowScene:wScene];
            ...
            _leeWindow = [[LEEAlertWindow alloc] init];
        }
        _leeWindow.frame = [[UIScreen mainScreen] bounds];
...
///!!!:自定义需求
@interface LEEItemImageView : UIImageView
@property (nonatomic, strong) LEEItem *item;

+ (LEEItemImageView *)imageView;
@end

@implementation LEEItemImageView

+ (LEEItemImageView *)imageView {
    return [[LEEItemImageView alloc] init];
}
...
- (void)setAction:(LEEAction *)action {
    ...
    if (action.highlight) {
        [self setTitle:action.highlight forState:UIControlStateHighlighted];
    }
    ...
    if (action.attributedHighlight) {
        [self setAttributedTitle:action.attributedHighlight forState:UIControlStateHighlighted];
    }
    
    if (action.font) {
        [self.titleLabel setFont:action.font];
    }
    ...
    if (action.highlightColor) {
        [self setTitleColor:action.highlightColor forState:UIControlStateHighlighted];
    }
    
    if (action.backgroundColor) {
        [self setBackgroundImage:[self getImageWithColor:action.backgroundColor] forState:UIControlStateNormal];
    }
    
    if (action.backgroundHighlightColor) {
        [self setBackgroundImage:[self getImageWithColor:action.backgroundHighlightColor] forState:UIControlStateHighlighted];
    }
    
    if (action.backgroundImage) {
        [self setBackgroundImage:action.backgroundImage forState:UIControlStateNormal];
    }
    
    if (action.backgroundHighlightImage) {
        [self setBackgroundImage:action.backgroundHighlightImage forState:UIControlStateHighlighted];
    }
    
    if (action.borderColor) {
        [self setBorderColor:action.borderColor];
    }
    
    if (action.borderWidth > 0) {
        [self setBorderWidth:action.borderWidth < DEFAULTBORDERWIDTH ? DEFAULTBORDERWIDTH : action.borderWidth];
    } else {
    ...
    if (action.height) {
        [self setActionHeight:action.height];
    }
    
    if (action.cornerRadius) {
        [self.layer setCornerRadius:action.cornerRadius];
    }
    ...
    if (action.borderPosition & LEEActionBorderPositionTop &&
        ...
        if (action.borderPosition & LEEActionBorderPositionBottom) {
            [self addBottomBorder];
        } else {
            [self removeBottomBorder];
        }
        
        if (action.borderPosition & LEEActionBorderPositionLeft) {
            [self addLeftBorder];
        } else {
            [self removeLeftBorder];
        }
        
        if (action.borderPosition & LEEActionBorderPositionRight) {
            [self addRightBorder];
        } else {
            [self removeRightBorder];
        }
...
- (void)layoutSubviews {
    ...
    if (_topLayer) {
        _topLayer.frame = CGRectMake(0, 0, self.frame.size.width, self.borderWidth);
    }
    
    if (_bottomLayer) {
        _bottomLayer.frame = CGRectMake(0, self.frame.size.height - self.borderWidth, self.frame.size.width, self.borderWidth);
    }
    
    if (_leftLayer) {
        _leftLayer.frame = CGRectMake(0, 0, self.borderWidth, self.frame.size.height);
    }
...
- (UIWindow *)currentKeyWindow {
    if (!_currentKeyWindow) {
        _currentKeyWindow = [LEEAlert shareManager].mainWindow;
    }
...
//        _currentKeyWindow = [UIApplication sharedApplication].keyWindow;
    ...
    if (_currentKeyWindow) {
        if (![LEEAlert shareManager].mainWindow) {
            [LEEAlert shareManager].mainWindow = _currentKeyWindow;
        }
    }
...
@property (nonatomic, weak) UIButton *closeBtn; ///< 右上角关闭按钮
...
- (void)updateAlertItemsLayout {
    ...
    [self.alertItemArray enumerateObjectsUsingBlock:^(id  _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
        @le_strongify(self);
        if (idx == 0) {
            self->alertViewHeight += self.config.modelHeaderInsets.top;
        }
        ...
        if ([item isKindOfClass:UIView.class]) {
            ...
            if ([item isKindOfClass:[UIImageView class]]) {
                UIImageView *imageV = (UIImageView *)item;
                viewFrame.size = imageV.image.size;
                viewFrame.origin.x = (alertViewMaxWidth - imageV.image.size.width - view.item.insets.left - view.item.insets.right) * 0.5;
            }
...
- (void)configAlert {
    ...
    [_containerView addSubview:({
        ...
        NSURL *url = [bundleName URLForResource:NSStringFromClass([LEEAlert class]) withExtension:@"bundle"];
        NSBundle *imgBundle = [NSBundle bundleWithURL:url];
        NSInteger screenScale = (NSInteger)([UIScreen mainScreen].scale);
        screenScale = screenScale > 2 ? 3 : screenScale;
        NSString *imgPath = [imgBundle pathForResource:[NSString stringWithFormat:@"close@%zix", screenScale] ofType:@"png"];
        ...
        _closeBtn = btn;
        _closeBtn.hidden = !self.config.modelIsShouldShowCloseBtn;
        btn;
    })];
    ...
    [self.config.modelItemArray enumerateObjectsUsingBlock:^(id _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        switch (item.type) {
            case LEEItemTypeImage: {
                LEEItemImageView *imageV = [LEEItemImageView imageView];
                imageV.contentMode = UIViewContentModeScaleAspectFill;
                
                [self.alertView addSubview:imageV];
                [self.alertItemArray addObject:imageV];
                
                [imageV sizeThatFits:imageV.image.size];
                
                void(^block)(UIImageView *imageView) = item.block;
                if (block) {
                    block(imageV);
                }
...
//                };
    ...
    [self.config.modelActionArray enumerateObjectsUsingBlock:^(id item, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (!action.backgroundHighlightColor) {
            action.backgroundHighlightColor = action.backgroundHighlightColor = [UIColor colorWithWhite:0.97 alpha:1.0f];
        }
        ...
        if (!action.borderWidth) {
            action.borderWidth = DEFAULTBORDERWIDTH;
        }
        
        if (!action.borderPosition) {
            action.borderPosition = (self.config.modelActionArray.count == 2 && idx == 0) ? LEEActionBorderPositionTop | LEEActionBorderPositionRight : LEEActionBorderPositionTop;
        }
    ...
    [self showAnimationsWithCompletionBlock:^{
        if (self) [self updateAlertLayout];
    }];
}

#pragma mark - 关闭按钮的action, 按钮的显示隐藏在view创建的时候就处理了, 不用判断self.config.modelIsShouldShowCloseBtn
///!!!:自定义需求
...
- (void)headerTapAction:(UITapGestureRecognizer *)tap {
    if (self.config.modelIsClickHeaderClose) {
        [self closeAnimationsWithCompletionBlock:nil];
    }
}
...
- (void)showAnimationsWithCompletionBlock:(void (^)(void))completionBlock {
    ...
    if (self.config.modelOpenAnimationStyle & LEEAnimationStyleFade) {
        self.containerView.alpha = 0.0f;
    }
    
    if (self.config.modelOpenAnimationStyle & LEEAnimationStyleZoomEnlarge) {
        self.containerView.transform = CGAffineTransformMakeScale(0.6f, 0.6f);
    }
    
    if (self.config.modelOpenAnimationStyle & LEEAnimationStyleZoomShrink) {
        self.containerView.transform = CGAffineTransformMakeScale(1.2f, 1.2f);
    }
    
    if (self.config.modelOpenAnimationConfigBlock) {
        @le_weakify(self);
        self.config.modelOpenAnimationConfigBlock(^{
            @le_strongify(self);
            if (!self) return ;
            
            if (self.config.modelBackgroundStyle == LEEBackgroundStyleTranslucent) {
                self.view.backgroundColor = [self.view.backgroundColor colorWithAlphaComponent:self.config.modelBackgroundStyleColorAlpha];
            } else if (self.config.modelBackgroundStyle == LEEBackgroundStyleBlur) {
                self.backgroundVisualEffectView.effect = [UIBlurEffect effectWithStyle:self.config.modelBackgroundBlurEffectStyle];
            }
...
- (void)closeAnimationsWithCompletionBlock:(void (^)(void))completionBlock {
    ...
    if (self.config.modelCloseAnimationConfigBlock) self.config.modelCloseAnimationConfigBlock(^{
        ...
        if (self.config.modelCloseAnimationStyle & LEEAnimationStyleOrientationNone) {
            containerFrame.origin.x = (viewWidth - containerFrame.size.width) * 0.5f;
            containerFrame.origin.y = (viewHeight - containerFrame.size.height) * 0.5f;
        } else if (self.config.modelCloseAnimationStyle & LEEAnimationStyleOrientationTop) {
            containerFrame.origin.x = (viewWidth - containerFrame.size.width) * 0.5f;
            containerFrame.origin.y = 0 - containerFrame.size.height;
        } else if (self.config.modelCloseAnimationStyle & LEEAnimationStyleOrientationBottom) {
            containerFrame.origin.x = (viewWidth - containerFrame.size.width) * 0.5f;
            containerFrame.origin.y = viewHeight;
        } else if (self.config.modelCloseAnimationStyle & LEEAnimationStyleOrientationLeft) {
            containerFrame.origin.x = 0 - containerFrame.size.width;
            containerFrame.origin.y = (viewHeight - containerFrame.size.height) * 0.5f;
        } else if (self.config.modelCloseAnimationStyle & LEEAnimationStyleOrientationRight) {
            containerFrame.origin.x = viewWidth;
            containerFrame.origin.y = (viewHeight - containerFrame.size.height) * 0.5f;
        }
        ...
        if (self.config.modelCloseAnimationStyle & LEEAnimationStyleFade) {
            self.containerView.alpha = 0.0f;
        }
        
        if (self.config.modelCloseAnimationStyle & LEEAnimationStyleZoomEnlarge) self.containerView.transform = CGAffineTransformMakeScale(1.2f, 1.2f);
        
        if (self.config.modelCloseAnimationStyle & LEEAnimationStyleZoomShrink) self.containerView.transform = CGAffineTransformMakeScale(0.6f, 0.6f);
...
- (NSMutableArray *)alertItemArray {
    if (!_alertItemArray) {
        _alertItemArray = [NSMutableArray array];
    }
...
- (void)updateActionSheetLayoutWithViewWidth:(CGFloat)viewWidth ViewHeight:(CGFloat)viewHeight
{
    ...
    [self.actionSheetItemArray enumerateObjectsUsingBlock:^(id  _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (item == self.actionSheetItemArray.lastObject) {
...
- (void)configActionSheet {
    ...
    [self.config.modelItemArray enumerateObjectsUsingBlock:^(id _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        switch (item.type) {
            case LEEItemTypeTitle: {
                ...
                label.textChangedBlock = ^{
                    if (self) [self updateActionSheetLayout];
                };
            } break;
            case LEEItemTypeContent: {
                void(^block)(UILabel *label) = item.block;
    ...
    for (id item in self.config.modelActionArray) {
        ...
        if (!action.borderColor) {
            action.borderColor = [UIColor colorWithWhite:0.86 alpha:1.0f];
        }
        ...
        self.config.modelOpenAnimationConfigBlock(^{
            ...
            switch (self.config.modelBackgroundStyle) {
                ...
                case LEEBackgroundStyleTranslucent: {
                    self.view.backgroundColor = [self.config.modelBackgroundColor colorWithAlphaComponent:self.config.modelBackgroundStyleColorAlpha];
                } break;
            ...
            self.isShowing = NO;
            [self.view setUserInteractionEnabled:YES];
            if (self.openFinishBlock) self.openFinishBlock();
...
- (void)closeAnimationsWithCompletionBlock:(void (^)(void))completionBlock {
    ...
    @le_weakify(self);
    if (self.config.modelCloseAnimationConfigBlock) {
        self.config.modelCloseAnimationConfigBlock(^{
            @le_strongify(self);
            if (!self) return ;
            ...
            if (self.config.modelCloseAnimationStyle & LEEAnimationStyleZoomEnlarge) {
                self.containerView.transform = CGAffineTransformMakeScale(1.2f, 1.2f);
            }
            if (self.config.modelCloseAnimationStyle & LEEAnimationStyleZoomShrink) {
                self.containerView.transform = CGAffineTransformMakeScale(0.6f, 0.6f);
            }
...
- (NSMutableArray <id>*)actionSheetItemArray {
    if (!_actionSheetItemArray) {
        _actionSheetItemArray = [NSMutableArray array];
    }
    return _actionSheetItemArray;
}

- (NSMutableArray <LEEActionButton *>*)actionSheetActionArray {
    if (!_actionSheetActionArray) {
        _actionSheetActionArray = [NSMutableArray array];
    }
    ...
    if (self) {
        ...
        self.config.modelFinishConfig = ^{
            ...
            if ([LEEAlert shareManager].queueArray.count) {
                ...
                if (!self.config.modelIsQueue && last.config.modelQueuePriority > self.config.modelQueuePriority) return;
                
                if (!last.config.modelIsQueue && last.config.modelQueuePriority <= self.config.modelQueuePriority) {
                    [[LEEAlert shareManager].queueArray removeObject:last];
                }
                
                if (![[LEEAlert shareManager].queueArray containsObject:self]) {
                    [[LEEAlert shareManager].queueArray addObject:self];
                ...
                if ([LEEAlert shareManager].queueArray.lastObject == self) {
                    [self show];
                }
            } else {
                [self show];
...
- (void)show {
    ...
    [LEEAlert shareManager].viewController.closeFinishBlock = ^{
        ...
        if ([LEEAlert shareManager].queueArray.lastObject == strongSelf) {
            ...
            if (strongSelf.config.modelIsContinueQueueDisplay) {
        ...
        if (strongSelf.config.modelCloseComplete) {
            strongSelf.config.modelCloseComplete();
        }
    };
}

- (void)closeWithCompletionBlock:(void (^)(void))completionBlock {
    if ([LEEAlert shareManager].viewController) {
        [[LEEAlert shareManager].viewController closeAnimationsWithCompletionBlock:completionBlock];
    }
}
